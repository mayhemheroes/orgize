...
# Rust maps the C/C++ $SANITIZER_FLAGS contract onto RUSTFLAGS (rustc ignores clang
# flags): ASan halts by default in rustc's -Zsanitizer=address, matching the halting
# contract of $SANITIZER_FLAGS. Debug-info contract (§6.2 item 10, DWARF < 4) comes
# in via $RUST_DEBUG_FLAGS (default -Zdwarf-version=3 — plain rustc -g emits DWARF-5).
: "${RUST_DEBUG_FLAGS:=-Zdwarf-version=3}"
export RUSTFLAGS="${RUSTFLAGS:-} --cfg fuzzing -Zsanitizer=address -Cdebuginfo=1 -Cforce-frame-pointers ${RUST_DEBUG_FLAGS}"
# libfuzzer-sys compiles libFuzzer's C++ via the cc crate — keep those CUs DWARF-3 too.
...
# non-sanitized build) — so mayhem/test.sh only RUNS it. This also populates the
# cargo registry cache with the dev-dependencies (criterion/insta/...) so the
# offline re-run and mayhem/test.sh resolve everything from $CARGO_HOME.
echo "=== building upstream test suite (cargo test -p orgize --all-features --no-run, normal flags) ==="
( unset RUSTFLAGS; cargo test -p orgize --all-features --no-run )
